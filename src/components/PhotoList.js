...
const PhotoList = ({ albumId }) => {
  ...
  const renderAlbums = () => {
    return photos.map(photo =>
      <PhotoDetail key={photo.title} photoId={photo.id} title={photo.title} imageUrl={`https://farm${photo.farm}.staticflickr.com/${photo.server}/${photo.id}_${photo.secret}.jpg`} />
    );
  }
